...
from zoneheal.core.trainer import AdaptiveBuckets, Trainer, grid_quadrant, regime_health, single_bucket
from zoneheal.envs.probes import GridLake, RegimeBandit
from zoneheal.heal.healing import HealLog
...
def ok_metric(env_name, env, policy):
    ...
    m = env.means
    gap = m.max(axis=1, keepdims=True) - m
    online = (policy.snapshot() * gap).sum(axis=1)
    return correct >= 1.0, {
...
def main():
    ...
    variants = ["static", "self_heal"]
    if args.env in ("grid", "both"):
        env_name = "grid"
        res = []
        for seed in range(args.seeds):
            for v in variants:
                res.append(run_one((env_name, v, seed, 1e-3, 1.0, ENVS[env_name]["iters"], ENVS[env_name]["fault_at"], args.out)))
        summarise(env_name, res, variants)
