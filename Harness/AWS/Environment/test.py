...
class ZoneEnv:
    def __init__(self, d, split="train", seed=0, episode_min=EPISODE_MIN):
        ...
        self.fault = None
        self.fixed_start = None # WHy because running this on random windo cause random window sizes and false healing 
    ...
    # This is the simulator not the harness 
    ...
    def step(self, action):
        ...
        own_p99 = sum(self.rows[zi][k] * (p99[k] + (RTT_MS if k != zi else 0.0)) for k in range(4)) # Change 1 -> Drops are occuring in each and every Bucket , thats because the reward is system wide and not for each bucket so we are localzing the p99 to per state 
        slo_missed = own_p99 > SLO_MS
        # Every bucket feels the brownout seo there nothign regions so we are making nit neighbourhood based 
...
# expect 0, 1, 2
    ...
    env.reset(seed=1, start_minute=16183)
    _, _, _, _, before = env.step(0)
    env.set_fault("brownout", zone=1, severity=1.0)     # zone index 1 = cluster 2
    _, _, _, _, after = env.step(0)
    print(round(before["p99_ms"], 1), "->", round(after["p99_ms"], 1), "|", after["fault"])
